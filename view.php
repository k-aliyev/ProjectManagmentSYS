...
if(isset($_GET["id"])){
    ...
    $stmt2 = $db->prepare("select * from project_files where project_name= ?");
    $stmt2->execute([$_GET["name"]]);
    $project_files = $stmt2->fetch(PDO::FETCH_ASSOC);

    $stmt3 = $db->prepare("select * from comments where pid= ?");
    ...
    <div class="row">
        <div class="col-md-7">
            ...
            <div class="project-info-box mt-4">
                ...
                <br>
                <?php 
                    if( isset($_SESSION["user_role"]) && $_SESSION["user_role"] == "student" || isset($_SESSION["user_role"]) && $_SESSION["user_role"] == "instructor"){
                     echo "<div class='project-info-box mt-0'>
                           <h5>Resources:</h5>
                          <p class='mb-0'>
                          <a href='uploads/{$project_files['filename']}'> {$project_files['filename']}</a></p>
                         </div>";
                    }
                ?>
                <br>
